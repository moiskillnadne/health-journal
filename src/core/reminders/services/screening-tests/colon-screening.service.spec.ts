...
import { ColonScreeningReminderService } from './colon-screening.service'

jest.mock('../../reminders.logger')

describe('ColonScreeningReminderService', () => {
  let colonScreeningReminderService: ColonScreeningReminderService
  ...
  beforeEach(async () => {
    const module = await createTestingModule({
      providers: [ColonScreeningReminderService],
    })

    colonScreeningReminderService = module.get<ColonScreeningReminderService>(ColonScreeningReminderService)
      ...
      // Valid
      {
        ...
        user: {
          procedures: [
            ...
            {
              datetime: subMonths(new Date(), 6),
              procedure: { tag: Procedure.Cologuard },
            },
            {
              name: mockName,
              datetime: subMonths(new Date(), 8),
  ...
  describe('sendColonScreeningToScheduleReminder', () => {
    it('should call getUserRemindersByNotificationTypes() with correct params', async () => {
      await colonScreeningReminderService.sendColonScreeningToScheduleReminder()
  ...
  describe('sendColonScreeningToExpireReminder', () => {
    it('should call getUserRemindersByNotificationTypes() with correct params', async () => {
      await colonScreeningReminderService.sendColonScreeningToExpireReminder()
